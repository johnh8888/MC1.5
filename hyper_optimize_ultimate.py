...
def main():
    ...
    parser.add_argument('--max_trials_per_round', type=int, default=300)
    ...
    round_num = 1
    while True:
        study = optuna.create_study(direction='maximize', study_name=f'ultimate_{round_num}',
                                    storage='sqlite:///optuna_ultimate.db', load_if_exists=True,
                                    pruner=optuna.pruners.MedianPruner(n_warmup_steps=50))
        study.optimize(lambda t: objective(t, issues), n_trials=args.max_trials_per_round, show_progress_bar=True)
        best_p = study.best_params
        score, r1, r2, r4, ms1, ms2, ms4 = evaluate(issues, best_p)
        print(f"Round {round_num}: 一生肖={r1:.3f}(连空{ms1}) 二生肖={r2:.3f}(连空{ms2}) 特别生肖={r4:.3f}(连空{ms4})")
        if score > 0:
            print("🎉 达标！保存参数并退出。")
            ...
            print("未达标，自动增大搜索范围和试验次数，进入下一轮...")
            args.max_trials_per_round += 200  # 下一轮更多试验
            # 可选择性调整提示，但不改代码
            round_num += 1
