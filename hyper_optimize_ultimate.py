...
def pred_two(hist):
    ...
    for i in range(len(hist)):
        _, nums, sp = hist[-(i + 1)]
        for z in ZODIAC_MAP:
            omission[z] = omission.get(z, i + 1) if omission[z] == 0 else omission[z]
        for n in nums:
            omission[get_zodiac(n)] = 0
        omission[get_zodiac(sp)] = 0
...
def main():
    ...
    while True:
        study = optuna.create_study(
            direction='maximize',
            study_name=f'ultimate_{round_num}',
            storage='sqlite:///optuna_ultimate.db',
            load_if_exists=True,
            pruner=optuna.pruners.MedianPruner(n_warmup_steps=50),
        )
        study.optimize(
            lambda t: objective(t, issues),
            n_trials=args.max_trials_per_round,
            show_progress_bar=True,
        )
        best_p = study.best_params
        score, r1, r2, r4, ms1, ms2, ms4 = evaluate(issues, best_p)
        print(
            f"Round {round_num}: 一生肖={r1:.3f}(连空{ms1}) "
            f"二生肖={r2:.3f}(连空{ms2}) 特别生肖={r4:.3f}(连空{ms4})"
        )
        if score > 0:
            print("🎉 达标！保存参数并退出。")
