...
def load_issues(conn, recent=200):  # 扩大历史范围
...
def evaluate(issues, params):
    ...
    # 强力惩罚：低于目标大幅扣分
    if r1 < 0.85: score *= 0.6
    if r2 < 0.95: score *= 0.7
    if r4 < 0.85: score *= 0.6
    return score
...
def main():
    ...
    study = optuna.create_study(direction='maximize', study_name='zodiac_v5',
                                storage='sqlite:///optuna_v5.db', load_if_exists=True,
