...
"""
全自动策略进化优化器（全量策略版）
一次性包含一生肖6策略、二生肖6策略、四生肖5策略、特别号8策略
"""
...
def single_weighted(hist, wsize, rec_w, safe_th):
    ...
    if max(scores.values())<safe_th:
        ...
        for i in range(len(recent)):
            ...
            for z in ZODIAC_MAP: omission[z] = i+1 if omission[z]==0 else omission[z]
...
def single_pure_cold(hist, wsize, rec_w, safe_th):
    omission = {z:0 for z in ZODIAC_MAP}
    recent = hist[-wsize:] if len(hist)>=wsize else hist
    for i,(_,nums,sp) in enumerate(recent):
...
def two_neighbor_pair(hist):
    ...
    recent_specials = [get_zodiac(sp) for _,_,sp in hist[-8:]]
    hot = Counter(recent_specials).most_common(1)[0][0] if recent_specials else "蛇"
    if pair_z == hot:
        hot = [z for z in ZODIAC_MAP if z!=pair_z][0]
    return [pair_z, hot]
...
# ============== 特别号策略 (8) ==============
def special_cold_neighbor(hist, target_issue_no, zodiac_pool, params, top_n=3):
...
def special_tail_focus(hist, target_issue_no, zodiac_pool, params, top_n=3):
    recent_specials = [row[2] for row in hist[-12:][::-1]]
    if not recent_specials: return [1,2,3]
    latest = recent_specials[0]
...
def special_omission_only(hist, target_issue_no, zodiac_pool, params, top_n=3):
    recent_specials = [row[2] for row in hist[-12:][::-1]]
    if not recent_specials: return [1,2,3]
    omission = {}
    for i,sp in enumerate(recent_specials):
...
def special_zone_bias(hist, target_issue_no, zodiac_pool, params, top_n=3):
    recent_specials = [row[2] for row in hist[-12:][::-1]]
    if not recent_specials: return [1,2,3]
...
def special_neighbor_tail(hist, target_issue_no, zodiac_pool, params, top_n=3):
    recent_specials = [row[2] for row in hist[-12:][::-1]]
    if not recent_specials: return [1,2,3]
    latest = recent_specials[0]
    omission = {}
...
def special_mixed_2cold1hot(hist, target_issue_no, zodiac_pool, params, top_n=3):
    recent_specials = [row[2] for row in hist[-12:][::-1]]
    if not recent_specials: return [1,2,3]
    omission = {}
    for i,sp in enumerate(recent_specials):
...
def special_omit_break(hist, target_issue_no, zodiac_pool, params, top_n=3):
    recent_specials = [row[2] for row in hist[-12:][::-1]]
    if not recent_specials: return [1,2,3]
    latest = recent_specials[0]
    omission = {}
...
def special_random_baseline(hist, target_issue_no, zodiac_pool, params, top_n=3):
    candidates = list(set(n for z in zodiac_pool for n in ZODIAC_MAP.get(z,[])))
    if not candidates: return [1,2,3]
...
def evaluate(issues, params, debug=False):
    ...
    for i in range(recent10_start, total):
        ...
        if s in cur_zod:
            ...
            single_streak += 1
            max_single = max(max_single, single_streak)
        ...
        if any(z in cur_zod for z in two):
            ...
            two_streak += 1
            max_two = max(max_two, two_streak)
        ...
        if hit:
            ...
            special_streak += 1
            max_special = max(max_special, special_streak)

        if debug and i >= recent10_start:
            print(f"[调试] {issues[i][0]} 单:{params['single_strategy']} 二:{params['two_strategy']} 四:{params['four_strategy']} 特:{params['special_strategy']} | 预测:{sp_picks} 实际:{cur_sp} {'√' if hit else '×'}")
...
def main():
    ...
    print(f"\n最优策略: 单:{best_p['single_strategy']} 双:{best_p['two_strategy']} 四:{best_p['four_strategy']} 特:{best_p['special_strategy']}")
