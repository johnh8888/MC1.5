...
STRATEGIES_SINGLE = {"weighted": single_weighted, "pure_hot": single_pure_hot, "pure_cold": single_pure_cold}
STRATEGIES_TWO = {"hot_cold": two_hot_cold, "double_hot": two_double_hot, "double_cold": two_double_cold}
STRATEGIES_FOUR = {"boosted": four_boosted, "momentum": four_momentum}
...
def evaluate(issues, params, debug=False):
    ...
    for i in range(recent10_start, total):
        ...
        base_four = four_boosted(past, params['four_boost']) if f_strat=="boosted" else four_momentum(past, params.get('momentum_w',1.0))
        ...
        if debug and i>=recent10_start:
            print(f"[调试] {issues[i][0]} 单:{params['single_strategy']} 二:{params['two_strategy']} 四:{params['four_strategy']} 特:{params['special_strategy']} | 预测特号:{sp_picks} 实际:{cur_sp} {'√' if hit else '×'}")
